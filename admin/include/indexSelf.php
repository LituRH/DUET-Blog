...
if($postid!="")
{
$sql= "SELECT * FROM posts WHERE (cat_id='$postid' && user_id='$id') ORDER BY post_id DESC ";	
}
else
{
//2. main sql query
...
//2. main sql query
$sql = "select* from posts WHERE user_id='$reqid' ORDER BY post_id DESC";
}
...
<?php 
 ...
 <div class="post">
   <form name="first" method="post" action="recentpost.php">
				...
				<p class="byline">Posted on <?php echo $row['date']; ?> &nbsp; <?php echo $row['time']; ?>&nbsp; by <?php echo "<a href='checkProfile.php?id=$id'>$name</a>";?> </a></p>
				<div class="entry"> 
			...
			<p class="byline1" readonly style="background-color:#FFF; color:#000000; border:0px; backface-visibility:hidden; text-justify:auto; text-decoration:none; font-size:16px" wrap="physical"> <?php echo $string; ?> <?php echo "<a href='fullPostRead.php?id=$row[0]'>.....Read More</a>";?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php if($row[5]==$id) {echo "<a href='indexSelf.php?id=$row[0]'>Delete</a>";}?>&nbsp;&nbsp;&nbsp;&nbsp;<?php if($row[5]==$id) {echo "<a href='updatePost.php?id=$row[0]'>Edit</a>";}?></p>
